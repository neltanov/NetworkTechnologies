...
void Socks5Proxy::handleGreeting() {
    ...
    async_read(client_socket, buffer(buffer_data, 2), bind(&Socks5Proxy::readAuthMethods, this, _1, _2));
}

void Socks5Proxy::readAuthMethods(const error_code& ec, std::size_t length) {
    if (!ec && length == 2 && buffer_data[0] == 5) {
        ...
        async_read(client_socket, buffer(buffer_data, count_of_methods), 
                    bind(&Socks5Proxy::sendServerChoice, this, _1, _2, count_of_methods));
    }
    else {
...
void Socks5Proxy::sendServerChoice(const error_code& ec, std::size_t length, std::size_t count_of_methods) {
    if (!ec && length == count_of_methods) {
        ...
        async_write(client_socket, buffer(buffer_data, 2), bind(&Socks5Proxy::handleConnectionRequest, this, _1, _2));
    }
    else {
...
void Socks5Proxy::handleConnectionRequest(const error_code& ec, std::size_t) {
    if (!ec) {
        // Reading client connection request (Firstly, SOCKS version, command code and reserved byte)
        async_read(client_socket, buffer(buffer_data, 3), bind(&Socks5Proxy::readAddressType, this, _1, _2));
    }
    else {
...
void Socks5Proxy::readAddressType(const error_code& ec, std::size_t length) {
    if (!ec && length == 3 && buffer_data[0] == 5 && buffer_data[1] == 1 && buffer_data[2] == 0) {
        // Reading address type
        async_read(client_socket, buffer(buffer_data, 1), bind(&Socks5Proxy::handleAddress, this, _1, _2));
    }
    else {
...
void Socks5Proxy::handleAddress(const error_code& ec, std::size_t length) {
    if (!ec && length == 1) {
        uint8_t address_type = buffer_data[0];
        if (address_type == 1) { // IPv4 address
            // Read 4 bytes address and 2 bytes port
            async_read(client_socket, buffer(buffer_data, 6), bind(&Socks5Proxy::readIPv4Address, this, _1, _2));
        }
...
void Socks5Proxy::readIPv4Address(const error_code& ec, std::size_t length) {
    if (!ec && length == 6) {
        ...
        auto remote_socket = std::make_shared<tcp::socket>(client_socket.get_executor());
        
        remote_socket->async_connect(tcp::endpoint(ip, port), bind(&Socks5Proxy::sendServerResponse, this, _1, remote_socket));
    }
}

void Socks5Proxy::sendServerResponse(const error_code& ec, std::shared_ptr<tcp::socket> remote_socket) {
    if (!ec) {
        ...
        async_write(client_socket, buffer(buffer_data, 10), 
                    bind(&Socks5Proxy::startDataTransfer, this, _1, _2, remote_socket));
    }
    else {
...
void Socks5Proxy::startDataTransfer(const error_code& ec, std::size_t, std::shared_ptr<tcp::socket> remote_socket) {
    if (!ec) {
        std::cout << "Data transfer from client " << client_socket.remote_endpoint() << std::endl;
        client_socket.async_read_some(buffer(buffer_data, max_length), bind(&Socks5Proxy::sendDataToServer, this, _1, _2, remote_socket));

        std::cout << "Data transfer from server " << remote_socket->remote_endpoint() << std::endl;
        remote_socket->async_read_some(buffer(buffer_data, max_length), bind(&Socks5Proxy::sendDataToClient, this, _1, _2, remote_socket));
    }
    else {
        std::cerr << ec.message() << std::endl;
    }
...
void Socks5Proxy::sendDataToServer(const error_code& ec, std::size_t length, std::shared_ptr<tcp::socket> remote_socket) {
    if (!ec) {
        std::cout << "Data transfer to server " << remote_socket->remote_endpoint() << std::endl;
        async_write(*remote_socket, buffer(buffer_data, length), bind(&Socks5Proxy::startDataTransfer, this, _1, _2, remote_socket));
    } else {
        // client_socket.close();
        // remote_socket->close();
...
void Socks5Proxy::sendDataToClient(const error_code& ec, std::size_t length, std::shared_ptr<tcp::socket> remote_socket) {
    if (!ec) {
        std::cout << "Data transfer to client " << client_socket.remote_endpoint() << std::endl;
        async_write(client_socket, buffer(buffer_data, length), bind(&Socks5Proxy::startDataTransfer, this, _1, _2, remote_socket));
    } else {
        // client_socket.close();
        // remote_socket->close();
